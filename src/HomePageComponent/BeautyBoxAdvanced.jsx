...
// Your positions for desktop and mobile
const items = [
  {
...
export default function BeautyBoxVisibleDescriptions() {
  ...
  useEffect(() => {
    ...
    imgRefs.current.forEach((imgRef) => {
      gsap.set(imgRef.current, {
        x: 0, y: 0, scale: 0.57, opacity: 0, rotation: -10,
      });
    });
    ...
    mm.add(
      ...
      (context) => {
        ...
        timeline.to(openImgRef.current, { scale: isMobile ? 1.08 : 1, duration: 0.24 });
        timeline.to(
          openImgRef.current,
        ...
        timeline.to(
          ...
          { opacity: 1, scale: isMobile ? 1.17 : 1, duration: 0.32, ease: "power2.inOut" },
          "<"
        );
        ...
        return () => {
          timeline.kill();
          imgRefs.current.forEach((imgRef) => gsap.killTweensOf(imgRef.current));
        };
      }
    );
  ...
  return (
    <div className="relative w-full mx-auto h-[70vh] md:h-[90vh] my-6 flex items-center justify-center
      bg-gradient-to-br from-[#f9e8dd] via-[#dba16b] to-[#B9743A] rounded-2xl overflow-hidden perspective-1000">
      ...
      {items.map((item, i) => (
        <React.Fragment key={item.title}>
          <img
            ...
            src={item.src}
            alt={item.alt}
            className="absolute w-32 h-20 md:w-56 md:h-36 object-cover rounded-xl shadow-lg left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 z-20 border-2 md:border-4 border-[#f9e8dd]"
            style={{
              willChange: "transform",
              transformOrigin: "center",
              boxShadow: "0 5px 25px 0 #dba16baa,0 0px 1.5px #fff",
            }}
          />
          <div
            ref={descRefs.current[i]}
            className={`absolute z-30 min-w-[8.5rem] max-w-[74vw] md:max-w-xs rounded-lg px-3 py-2 md:px-6 md:py-4 shadow-md text-xs md:text-sm border-2 border-[#dba16b] bg-[#f9e8dd]/90 ${item.descPosition}`}
            style={{
              willChange: "transform,opacity",
              color: "#222",
              boxShadow: "0 12px 28px -2px #dba16b55",
            }}
          >
            <div className="font-bold text-base md:text-lg text-[#8c4d21] mb-1">{item.title}</div>
            <div className="text-[#4e2d13]">{item.desc}</div>
          </div>
        </React.Fragment>
      ))}
    </div>
